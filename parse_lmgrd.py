...
# INPUT
    ...
    def read_lmgrd_file(self):
        ...
        if num_checks == 0:
            print ("\nWARNING: no license checkouts (OUT: and IN:) of Masterkey or MSC One features are found in file ...\n")
            status = -1
        return status, num_checks, num_starts
...
# POSTPROCESSING
    ...
    def post_data(self):
        ...
        # return value
        # for k,v in self.token_minutes.items():
        #     print (k, " - ", v)
        # print (" ============================= ")
    ...
    def csv_daily_token_minutes(self):
        ...
        with open(csv_01, 'w', encoding='utf-8') as f:
            for k,v in self.per_day_tok_min.items():
               ...
               line_02 = ""
               for k1,v1 in v.items():
                   k1_str = str(k1).replace(" ","").replace(",",":")
                   line_01 = line_01 + k1_str + delimiter 
        ...
        with open(csv_02, 'w', encoding='utf-8') as f:
            csv_all_features = [a.replace(" ", "") + ":" + str(b) for a, b in self.all_features ]
            header = "date" + delimiter + delimiter.join(map(str, csv_all_features))
            f.write(header + "\n")
